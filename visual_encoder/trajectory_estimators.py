...
def compute_trajectory(img_beg, img_end, x0, y0, method='pc', window_type=None):
    ...
    if method == 'pc':
        deltax, deltay = shift_estimators.pc_method(img_beg, img_end)
    elif method == 'svd':
        deltax, deltay = shift_estimators.svd_method(img_beg, img_end)
    else:
        raise ValueError('Opção de método não suportada.')
