...
def blackman_harris_window(size):
    ...
    n = np.arange(size)
    # Coeficientes do janelamento de Blackman-Harris
    a0 = 0.35875
    a1 = 0.48829
    a2 = 0.14128
    a3 = 0.01168
    # Cálculo da janela de Blackman-Harris
    window = a0 - a1 * np.cos(2 * np.pi * n / (size - 1)) + a2 * np.cos(4 * np.pi * n / (size - 1)) - a3 * np.cos(6 * np.pi * n / (size - 1))
    return window
...
def filter_array_by_threshold(arr, threshold):
    filtered_arr = []
    n = len(arr)
